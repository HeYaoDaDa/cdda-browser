...
data class Temperature(@MapInfo(ignore = true) var value: Long = 0) : CddaSubObject() {
  enum class Unit(val unit: Set<String>, val num: Long = 1) {
    K(setOf("k"))
  }

  override fun finalize(jsonValue: Any, param: String) {
    val defaultNum: Long = if (param.isBlank()) Unit.K.num else Unit.valueOf(param).num
    when (jsonValue) {
      is String -> {
